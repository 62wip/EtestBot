...
async def process_name(message: Message, state: FSMContext) -> None:
    if message.text[0] == '/':
        ...
        await message.answer(f'Отлично, <u>{await state.get_data().get("fio")}</u>! Теперь укажи <i>кто ты</i>.', parse_mode="HTML", reply_markup=kb.set_status)
...
async def process_name(message: Message, state: FSMContext) -> None:
    if message.text == 'Проподователь':
        ...
        await message.answer(f'Отлично, <u>{await state.get_data().get("fio")}</u>! Регестрация завершена.', parse_mode="HTML")
        ...
        await message.answer(f'Отлично, <u>{state.get_data()}</u>! Осталось установить <i>группу или класс</i>.', parse_mode="HTML", reply_markup=kb.set_status)
...
async def process_name(message: Message, state: FSMContext) -> None:    
    if message.text[0] == '/':
        ...
        await message.answer(f'Отлично, <u>{state.get_data()}</u>! Регестрация завершена.', parse_mode="HTML")
        context_data = await state.get_data()
        user = User(message.from_user.id, message.from_user.username, context_data.get('fio'), context_data.get('status'),context_data.get('group'))
        print(user)
        await state.finish()